...
public class InventoryRepository
{
    ...
    {
        const string sql = @"
            INSERT INTO inventory (asin, title, image_url, amazon_url, amazon_price, currency, is_active, last_scraped)
            VALUES (@asin, @title, @imageUrl, @amazonUrl, @amazonPrice, @currency, true, NOW())
            ON CONFLICT (asin)
            DO UPDATE SET
